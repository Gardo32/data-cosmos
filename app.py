...
def analyze_location():
    """Analyze vegetation at user's location"""
    try:
        # Get location from IP
        latitude, longitude, city, country = get_location()
        
        # Get satellite image for location
        img = get_satellite_image(latitude, longitude, zoom=15)
        if img is None:
            return jsonify({
                'status': 'error',
                'message': 'Could not retrieve satellite imagery'
            }), 500
        ...
        # Sample GLI for heatmap (downsample to 50x50 max)
        ...
        analysis_record = {
            ...
            'rgb_histogram': {
                'r_values': rgb_data['r'].tolist(),
                'g_values': rgb_data['g'].tolist(),
                'b_values': rgb_data['b'].tolist()
            },
            'heatmap_data': heatmap_data.tolist(),
            'historical_data': historical_data
        }
        # Fetch and add weather and pollen data
        try:
            weather_rt, weather_combined = get_combined_weather_data(city, latitude, longitude)
            if not weather_combined or weather_combined.empty:
        ...
        try:
            ...
            if not pollen_df.empty:
                # Store all pollen data in the analysis record
        ...
        # Store in memory (most recent 10)
